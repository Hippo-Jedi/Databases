...
Select film_id from film where length = ( Select max(length) from film) UNION Select film_id from film where rental_duration = ( Select min(rental_duration) from film) order by film_id;